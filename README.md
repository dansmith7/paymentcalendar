...
- `ALLOW_DEV_WRITES` (`false` по умолчанию; влияет только на Supabase-режим в dev, mock-режим использует отдельную локальную песочницу)
...
- В dev используется отдельная локальная песочница `.dev-data/mock-payment-store.json`: заявки и справочники читаются и пишутся туда, Supabase в mock-режиме не используется.
- Локальную песочницу можно сбросить, удалив папку `.dev-data/`; при следующем запуске seed-данные создадутся заново.
...
- Manager:
  ...
  - проведение и отмена оплат с автоматическим пересчётом статуса, фактически оплаченной суммы и остатка
  - управление плановой датой и группой финучёта
